...
namespace ZoneDepict
{
    public class ZDMap
    {
        ...
        static private List<ZDObject>[,] RecordGrid = new List<ZDObject>[ZDGameRule.MAP_WIDTH, ZDGameRule.MAP_HEIGHT];

        static public void Register(ZDObject Caller)
        {
            // -- Preparation --
            var UnitLoc = ZDGameRule.WorldToUnit(Caller.transform.position);
        ...
        {
            return new Vector2(x + ZDGameRule.MAP_WIDTH / 2,
                                            y + ZDGameRule.MAP_HEIGHT / 2);
        }
        ...
        {
            return !(Mathf.Abs(x) > (int)(ZDGameRule.MAP_WIDTH / 2) ||
                        Mathf.Abs(y) > (int)(ZDGameRule.MAP_HEIGHT / 2));
        }
